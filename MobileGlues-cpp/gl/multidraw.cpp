...
GLAPI GLAPIENTRY void mg_glMultiDrawElementsBaseVertex_compute(GLenum mode, GLsizei* counts, GLenum type,
    ...
    static GLsizeiptr g_ssbo_capacity = 0;

    if ((GLsizeiptr)primcount > g_ssbo_capacity) {
        g_ssbo_capacity = (GLsizeiptr)primcount * 2;
        GLsizeiptr cap = g_ssbo_capacity;

        GLES.glDeleteBuffers(1, &g_firstidx_ssbo);
        GLES.glGenBuffers(1, &g_firstidx_ssbo);
        GLES.glBindBuffer(GL_SHADER_STORAGE_BUFFER, g_firstidx_ssbo);
        GLES.glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(GLuint) * cap, nullptr, GL_DYNAMIC_DRAW);

        GLES.glDeleteBuffers(1, &g_basevtx_ssbo);
        GLES.glGenBuffers(1, &g_basevtx_ssbo);
        GLES.glBindBuffer(GL_SHADER_STORAGE_BUFFER, g_basevtx_ssbo);
        GLES.glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(GLint) * cap, nullptr, GL_DYNAMIC_DRAW);
    ...
    GLES.glBindBuffer(GL_SHADER_STORAGE_BUFFER, g_firstidx_ssbo);
    CHECK_GL_ERROR_NO_INIT
    GLES.glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLuint) * primcount, first_index.data());
    CHECK_GL_ERROR_NO_INIT

    GLES.glBindBuffer(GL_SHADER_STORAGE_BUFFER, g_basevtx_ssbo);
    CHECK_GL_ERROR_NO_INIT
    GLES.glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLint) * primcount, base_vtx.data());
    CHECK_GL_ERROR_NO_INIT

    GLES.glBindBuffer(GL_SHADER_STORAGE_BUFFER, g_prefixsumbuffer);
    CHECK_GL_ERROR_NO_INIT
    GLES.glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLuint) * primcount, g_prefix_sum.data());
    CHECK_GL_ERROR_NO_INIT
